...
namespace Servicios_Estudiantes.Api.Controladores.V1
{
    public static class EstudiantesV1
    {
        public static RouteGroupBuilder MapEstudiantes(this RouteGroupBuilder group)
        {
            ...
            {
                if (!AutorizacionAccesoEstudiante.PuedeGestionarPerfil(contexto.User, estudianteId))
                    return RespuestaAccesoApi.Prohibido();
            ...
            group.MapGet("{estudianteId:int}/inscripcion", async (
                HttpContext contexto,
                IMediator mediator,
                int estudianteId,
                bool soloActivas,
            ...
            group.MapPost("{estudianteId:int}/inscripcion/fila", async (
                HttpContext contexto,
                IMediator mediator,
                int estudianteId,
                InscripcionFilaCuerpo cuerpo,
                CancellationToken cancellationToken) =>
            {
                if (!AutorizacionAccesoEstudiante.PuedeGestionarPerfil(contexto.User, estudianteId))
                    return RespuestaAccesoApi.Prohibido();

                return Results.Ok(await mediator.Send(new AgregarInscripcionFilaCommand(estudianteId, cuerpo.MateriaId), cancellationToken));
            }).RequireAuthorization();

            group.MapDelete("{estudianteId:int}/inscripcion/materias/{materiaId:int}", async (
                HttpContext contexto,
                IMediator mediator,
                int estudianteId,
                int materiaId,
                CancellationToken cancellationToken) =>
            {
                ...
                return Results.Ok(await mediator.Send(new EliminarInscripcionFilaCommand(estudianteId, materiaId), cancellationToken));
            }).RequireAuthorization();

            group.MapPut("{estudianteId:int}/inscripcion/materia", async (
                HttpContext contexto,
                IMediator mediator,
                int estudianteId,
                CambiarMateriaCuerpo cuerpo,
                CancellationToken cancellationToken) =>
            {
                if (!AutorizacionAccesoEstudiante.PuedeGestionarPerfil(contexto.User, estudianteId))
                    return RespuestaAccesoApi.Prohibido();
                ...
                return Results.Ok(
                    await mediator.Send(
                        new CambiarMateriaInscripcionCommand(estudianteId, cuerpo.MateriaIdAnterior, cuerpo.MateriaIdNueva),
                        cancellationToken));
            }).RequireAuthorization();
