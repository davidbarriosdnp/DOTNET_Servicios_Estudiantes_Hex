...
public sealed class CasosUsoCatalogosPruebas
{
    ...
    {
        Mock<IRepositorioAcademico> repo = new();
        repo.Setup(r => r.InsertarMateriaAsync("Mat", 3, 2, 3, null, null, null, null, null, It.IsAny<CancellationToken>())).ReturnsAsync(8);

        CrearMateriaHandler crear = new(repo.Object);
        Assert.Equal(8, (await crear.Handle(new CrearMateriaCommand("Mat", 3, 2, 3, null, null, null, null, null), CancellationToken.None)).Resultado);

        ActualizarMateriaHandler act = new(repo.Object);
        await act.Handle(new ActualizarMateriaCommand(8, "Mat2", 3, 2, 3, null, null, null, null, null, 1), CancellationToken.None);

        EliminarMateriaHandler elim = new(repo.Object);
        await elim.Handle(new EliminarMateriaCommand(8), CancellationToken.None);

        MateriaDetalleDto det = new(8, "Mat", 3, 2, 3, FechaBase, null, 1, "Prof", null, null, null, null, null);
        ...
        MateriaCatalogoDto cat = new(8, "Mat", 3, 2, 3, "Prof", FechaBase, null, 1, null, "", "", null, null, null, null);
    ...
    public void CrearMateriaValidator_RechazaCreditosDistintosDeTres()
    {
        ...
        Assert.False(v.Validate(new CrearMateriaCommand("x", 0, 1, 1, null, null, null, null, null)).IsValid);
        Assert.False(v.Validate(new CrearMateriaCommand("x", 4, 1, 1, null, null, null, null, null)).IsValid);
        Assert.True(v.Validate(new CrearMateriaCommand("x", 3, 1, 1, null, null, null, null, null)).IsValid);
    }
}
